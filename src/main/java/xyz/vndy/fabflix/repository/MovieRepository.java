...
public interface MovieRepository extends JpaRepository<Movie, String> {
    ...
    @Query(value = "SELECT m.id AS movie_id, m.title AS movie_title, m.year AS movie_year, m.director AS movie_director,\n" +
            "       GROUP_CONCAT(DISTINCT g.name) AS genres_name, GROUP_CONCAT(DISTINCT s.name) AS stars_names, \n" +
            "       GROUP_CONCAT(DISTINCT s.id) AS stars_ids, r.rating AS ratings_rating\n" +
            "FROM movies m\n" +
